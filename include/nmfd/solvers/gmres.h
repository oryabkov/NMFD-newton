...
template <
    class VectorOperations, class Monitor, class Log, class LinearOperator,
    class Preconditioner        = preconditioners::dummy<VectorOperations, LinearOperator>,
    class ResidualRegulariation = detail::residual_regularization_dummy,
    class DenseOperations =
        detail::dense_operations<typename VectorOperations::Ord, typename VectorOperations::scalar_type>>
...
public:
    ...
    struct params : public logged_obj_params_t
    {
        ...
        unsigned
            batch_size; //size of the batch size s.t. Ritz vector converhence is checked each batch_size inside the krylov bass size
        ...
        params( const std::string &log_prefix = "", const std::string &log_name = "gmres::" )
            : logged_obj_params_t( 0, log_prefix + log_name ), basis_size( 20 ), batch_size( 5 ),
              preconditioner_side( 'R' ), reorthogonalization( false ), do_restart_on_false_ritz_convergence( false ),
        ...
        void from_json( const nlohmann::json &j )
        {
            ...
            do_restart_on_false_ritz_convergence =
                j.value( "do_restart_on_false_ritz_convergence", do_restart_on_false_ritz_convergence );
...
private:
    ...
    void
    calc_left_preconditioned_residual( const linear_operator_type &A, const T_vec &x, const T_vec &b, T_vec &r ) const
    {
...
public:
    ...
    gmres(
        std::shared_ptr<vector_operations_type> vec_ops, Log *log = nullptr, const params &prm = params(),
        ...
        : parent_t( std::move( vec_ops ), log, prm, prm.monitor, std::move( prec ) ), prms_( prm ),
          residual_reg_( std::move( residual_reg ) ), dense_ops_( std::move( dense_ops ) )
    {
    ...
    gmres(
        std::shared_ptr<const linear_operator_type> A, std::shared_ptr<vector_operations_type> vec_ops,
        Log *log = nullptr, const params &prm = params(), std::shared_ptr<preconditioner_type> prec = nullptr,
        std::shared_ptr<residual_regulaization_t> residual_reg = std::make_shared<residual_regulaization_t>(),
        std::shared_ptr<dense_operations_t>       dense_ops    = std::make_shared<dense_operations_t>()
    )
    ...
    gmres( const utils_hierarchy &utils, const params_hierarchy &prm = params_hierarchy() )
        : gmres(
              utils.vec_ops, utils.log, prm,
              nmfd::detail::algo_hierarchy_creator<preconditioner_type>::get(
                  utils.preconditioner, prm.preconditioner
              ),
            ...
            do
            {
                ...
                do
                {
                    ...
                    for ( int k = 0; k <= i; k++ )
                    {
                        ...
                        while ( ( prms_.reorthogonalization ) &&
                                ( c_norm > error_L2_basic_type_ * next_r_norm ) ) //iterative correction
                        {
                            ...
                            if ( correction_iterations > 10 )
                            {
                                ...
                                logged_obj_t::warning_f(
                                    "failed in Gram-Schmidt reorthogonalization in iteration %i, restart %i with error "
                                    "%e",
                                    k, i, c_norm
                                );
                                break;
                    ...
                    {
                        auto reduction_rate_prod = std::accumulate(
                            reduction_rates.begin(), reduction_rates.end(), 1.0, std::multiplies<T>()
                        );

                        logged_obj_t::info_f(
                            "iter = %i(%i), resid_estimate = %e, reduction = %.03f", total_iterations + 1, i + 1,
                            monitor_.norm_out( resid_estimate ),
                            std::pow( reduction_rate_prod, 1.0 / reduction_rates.size() )
                        );
                    }
                    ...
                    {
                        if ( prms_.do_restart_on_false_ritz_convergence )
                            break;
