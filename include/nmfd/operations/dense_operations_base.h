...
template<class Card, class Type, class DenseVectorType = scfd::arrays::tensor0_array_nd<Type, 1, scfd::memory::host>,
...
public:
    ...
    template<class... Args> void init_row_vectors(Args&&... args) const {
        check_init();
    ...
    template<class... Args> void init_col_vectors(Args&&... args) const {
        check_init();
    ...
    template<class... Args> void init_matrices(Args&&... args) const {
        check_init();
    ...
    void solve_upper_triangular_subsystem(const matrix_type& A, const vector_type& b, vector_type& x,
                                          const Card ind) const {
        if (ind > 0) {
