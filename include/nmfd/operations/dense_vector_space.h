...
template<class Type, int Dim, class DenseVectorType = scfd::arrays::tensor0_array_nd<Type, 1, scfd::memory::host>,
         class DenseMatrixType = scfd::arrays::tensor0_array_nd<Type, 2, scfd::memory::host>>
class dense_vector_space : public dense_operations_base<Type, DenseVectorType, DenseMatrixType> {
...
public:
    ...
    void assign_slices(const vector_type& x, const std::vector<std::pair<size_t, size_t>> slices,
                       vector_type& y) const {
        size_t index_y = 0;
