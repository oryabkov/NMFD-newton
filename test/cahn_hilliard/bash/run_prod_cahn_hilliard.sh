...
# Production run script for test_cahn_hilliard
# Runs all combinations of solver, preconditioner, and precision
#
# Usage: ./run_prod_cahn_hilliard.sh [arch] [grid_size] [output_base] [max_iterations] [gpu_device] [solver] [preconditioner]
#   arch: Architecture to use (cpu, omp, or cuda). Default: cuda
#   grid_size: Grid size (number of points per dimension). Default: 32
#   output_base: Base output directory. Default: conv_cahn_hilliard_prod
...
ARCH="${1:-cuda}"
GRID_SIZE="${2:-32}"
OUTPUT_BASE="${3:-conv_cahn_hilliard_prod}"
...
for float_type in f d; do
    ...
    if [ "$float_type" == "f" ]; then
        binary="test_cahn_hilliard_${ARCH}_f.bin"
        tolerance="$TOLERANCE_FLOAT"
        newton_tol="$NEWTON_TOL_FLOAT"
        type_label="float"
    else
        binary="test_cahn_hilliard_${ARCH}_d.bin"
...
echo "Each run has its own times.dat, nonlinear_history.dat, and convergence_history_N.dat files"
echo "=========================================="
