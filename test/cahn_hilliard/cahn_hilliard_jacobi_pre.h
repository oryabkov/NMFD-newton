#ifndef __CAHN_HILLIARD_JACOBI_PRE_H__
#define __CAHN_HILLIARD_JACOBI_PRE_H__
...
#include "kernels/cahn_hilliard_jacobi_pre.h"
...
<
    ...
    /**********************************************/
    class LinOp=cahn_hilliard_jacobi_op<VectorSpace, Log>,
    class Backend=typename VectorSpace::backend_type
>
class cahn_hilliard_jacobi_pre : public nmfd::preconditioners::preconditioner_interface<VectorSpace,LinOp>
{
...
public: // Especially for SYCL
    using preconditioner_kernel = kernels::cahh_hilliard_jacobi_preconditioner
    <
...
public:
    ...
    cahn_hilliard_jacobi_pre(const utils_hierarchy &u, const params_hierarchy &p) {}

    cahn_hilliard_jacobi_pre(
        ...
        step(grid_step), b_cond(cond), vector(vspace->get_range()) {}

    cahn_hilliard_jacobi_pre(std::shared_ptr<const lin_op_t> op){ set_operator(op); }
...
public:
    ...
    void apply(vector_type &v) const
    {
        ...
        for_each_nd_inst(preconditioner_kernel{v, vector, range, step, b_cond, D, gamma}, range);
    };
