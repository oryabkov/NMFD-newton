...
public:
    ...
    IterationLogger(
        ...
        std::shared_ptr<VectorSpace> vspace,
    ...
    void check_finished( int iters_performed, const vector_type &x, const vector_type &r ) override
    {
        ...
        // Write to CSV
        csv_stream_ << iters_performed << "," << std::scientific << std::setprecision( 6 ) << resid_norm << ","
                    << std::fixed << std::setprecision( 3 ) << elapsed_ms << std::endl;
...
void write_summary_csv( const SolverResults<Scalar> &results )
{
    ...
    if ( !file_exists )
    {
        summary_output << "solver,preconditioner,run_label,scalar_type,grid_size,"
                       << "converged,iterations,time_ms,initial_residual,final_residual,convergence_rate" << std::endl;
    }

    summary_output << results.solver_name << "," << results.preconditioner_type << "," << results.run_label << ","
                   << results.scalar_label << "," << results.grid_size << ","
                   << ( results.converged ? "true" : "false" ) << "," << results.iterations << "," << std::fixed
                   << std::setprecision( 3 ) << results.time_ms << "," << std::scientific << std::setprecision( 6 )
                   << results.initial_residual << "," << results.final_residual << "," << std::fixed
                   << std::setprecision( 6 ) << results.convergence_rate << std::endl;
}
...
void print_solver_summary( const SolverResults<Scalar> &r )
{
    ...
    std::cout << "║  Grid size:      " << std::setw( 4 ) << grid_size << " x " << std::setw( 4 ) << grid_size << " x "
              << std::setw( 4 ) << grid_size << "                              ║" << std::endl;
    std::cout << "║  DOFs:           " << std::setw( 12 ) << dofs << "                                  ║" << std::endl;
    std::cout << "╠══════════════════════════════════════════════════════════════════╣" << std::endl;
    ...
    std::cout << "║  Scalar type:    " << std::left << std::setw( 20 ) << r.scalar_label
              << "                            ║" << std::endl;
    std::cout << "╠══════════════════════════════════════════════════════════════════╣" << std::endl;
    std::cout << "║  Status:         " << std::left << std::setw( 20 )
              << ( r.converged ? "✓ CONVERGED" : "✗ NOT CONVERGED" ) << "                            ║" << std::endl;
    std::cout << "║  Iterations:     " << std::right << std::setw( 8 ) << r.iterations << " / " << std::setw( 8 )
              << r.max_iterations << "                            ║" << std::endl;
    std::cout << "║  Time:           " << std::right << std::setw( 12 ) << std::fixed << std::setprecision( 2 )
    ...
    std::cout << "║    Metrics CSV:  " << std::left << std::setw( 48 ) << r.csv_file.substr( 0, 48 ) << " ║"
              << std::endl;
    std::cout << "║    Log file:     " << std::left << std::setw( 48 ) << r.log_file.substr( 0, 48 ) << " ║"
              << std::endl;
    std::cout << "║    Summary:      " << std::left << std::setw( 48 ) << r.summary_file.substr( 0, 48 ) << " ║"
              << std::endl;
    std::cout << "╚══════════════════════════════════════════════════════════════════╝" << std::endl;
    std::cout << std::endl;
}
