...
int main(int argc, char const *args[]) {
  ...
  using dense_vector_space_t =
      nmfd::operations::dense_vector_space<T, vector_traits,
  ...
  std::shared_ptr<dense_vector_space_t> vec_space =
      std::make_shared<dense_vector_space_t>();
  vector_type x = {1, 2, 3};
  vector_type y = {4, 5, 6};
  ...
  // Test vector space dimensions
  {
    ...
    if (dim == Dim) {
      ...
      log.error("✗ `size()` method test failed. Expected " +
                std::to_string(Dim) + " but returned " + std::to_string(dim));
      failed_counter++;
    }
  }

  {
    ...
    if (vec_size == Dim) {
      ...
      log.error("✗ `get_size(x)` method test failed. Expected " +
                std::to_string(Dim) + " but returned " +
  ...
  // Test squared L2 norm
  {
    ...
    if (std::abs(norm - 14) < eps) // 1² + 2² + 3² = 1 + 4 + 9 = 14
    {
      ...
      log.error("✗ `norm_sq(x)` method test failed. Expected 14 but returned " +
                std::to_string(norm));
      failed_counter++;
    }
  }
  ...
  // Test L2 norm
  {
    ...
    if (std::abs(norm - std::sqrt(14)) < eps) {
      ...
      log.error("✗ `norm(x)` method test failed. Expected " +
                std::to_string(std::sqrt(14)) + " but returned " +
                std::to_string(norm));
      failed_counter++;
    }
  }
  ...
  // Test infinity norm (max absolute value)
  {
    ...
    if (std::abs(norm - 3) < eps) // max(|1|, |2|, |3|) = 3
    {
      ...
      log.error("✗ `norm_inf(x)` method test failed. Expected 3 but returned " +
                std::to_string(norm));
      failed_counter++;
    }
  }
  ...
  log.info("=== Testing Norm/Inner-Product Aliases ===");
  {
    ...
    T sp = vec_space->scalar_prod(xa, ya);
    T sp_l2 = vec_space->scalar_prod_l2(xa, ya);
    if (std::abs(sp - 32) < eps && std::abs(sp_l2 - sp) < eps) {
      log.info("✓ `scalar_prod_l2(x, y)` matches `scalar_prod(x, y)`");
      passed_counter++;
    } else {
      log.error("✗ `scalar_prod_l2` mismatch: sp=" + std::to_string(sp) +
                ", sp_l2=" + std::to_string(sp_l2));
      failed_counter++;
    }

    // norm2/norm_l2 should match norm; *_sq should match norm_sq
    T n = vec_space->norm(xa);
    T n2 = vec_space->norm2(xa);
    T nl2 = vec_space->norm_l2(xa);
    T nsq = vec_space->norm_sq(xa);
    T n2sq = vec_space->norm2_sq(xa);
    T nl2sq = vec_space->norm_l2_sq(xa);
    if (std::abs(n - std::sqrt(14)) < eps && std::abs(n2 - n) < eps &&
        std::abs(nl2 - n) < eps && std::abs(nsq - 14) < eps &&
        std::abs(n2sq - nsq) < eps && std::abs(nl2sq - nsq) < eps) {
    ...
    T a1 = vec_space->asum(xb);
    T n1 = vec_space->norm1(xb);
    T nl1 = vec_space->norm_l1(xb);
    if (std::abs(a1 - 6) < eps && std::abs(n1 - a1) < eps &&
        std::abs(nl1 - a1) < eps) {
    ...
    T ni = vec_space->norm_inf(xb);
    T nli = vec_space->norm_l_inf(xb);
    ...
    if (std::abs(vec_space->sum(xa) - 6) < eps) {
  ...
  // Test setting value at specific index
  {
    ...
    if ((tmp_x[0] - 10) < eps && (tmp_x[1] - 2) < eps && (tmp_x[2] - 3) < eps) {
      ...
      log.error("✗ `set_value_at_point(val_x, at, x)` method test failed. "
                ...
                std::to_string(tmp_x[0]) + ", " + std::to_string(tmp_x[1]) +
                ", " + std::to_string(tmp_x[2]) + "}");
      failed_counter++;
    }
  }
  ...
  // Test assigning scalar to all elements
  {
    ...
    if ((tmp_x[0] - 10) < eps && (tmp_x[1] - 10) < eps &&
        (tmp_x[2] - 10) < eps) {
  ...
  {
    vector_type tmp_x = {1, 2, 3};
    vec_space->add_mul_scalar(
        10, 2, tmp_x); // x = 2*x + 10 = {2*1+10, 2*2+10, 2*3+10} = {12, 14, 16}
    if ((tmp_x[0] - 12) < eps && (tmp_x[1] - 14) < eps &&
        (tmp_x[2] - 16) < eps) {
  ...
  // Test vector assignment: y = x
  {
    ...
    if ((tmp_y[0] - 1) < eps && (tmp_y[1] - 2) < eps && (tmp_y[2] - 3) < eps) {
      ...
      log.error(
          ...
          std::to_string(tmp_y[0]) + ", " + std::to_string(tmp_y[1]) + ", " +
          std::to_string(tmp_y[2]) + "}");
      failed_counter++;
    }
  }
  ...
  // Test linear combination assignment: z = mul_x * x + mul_y * y
  {
    ...
    vec_space->assign_lin_comb(
        2, tmp_x, 3, tmp_y,
        tmp_z); // z = 2*{1,2,3} + 3*{4,5,6} = {2,4,6} + {12,15,18} = {14,19,24}
    if ((tmp_z[0] - 14) < eps && (tmp_z[1] - 19) < eps &&
        (tmp_z[2] - 24) < eps) {
      ...
      log.error("✗ `assign_lin_comb(mul_x, x, mul_y, y, z)` method test "
                ...
                std::to_string(tmp_z[0]) + ", " + std::to_string(tmp_z[1]) +
                ", " + std::to_string(tmp_z[2]) + "}");
      failed_counter++;
    }
  }
  ...
  // Test add-multiply: y = mul_x * x + mul_y * y
  {
    ...
    if ((tmp_y[0] - 14) < eps && (tmp_y[1] - 19) < eps &&
        (tmp_y[2] - 24) < eps) {
  ...
  // Test add-multiply: z = mul_x * x + mul_y * y + mul_z * z
  {
    ...
    vec_space->add_lin_comb(
        ...
        tmp_z); // z = 2*{1,2,3} + 3*{4,5,6} + 4*{7,8,9} = {2,4,6} + {12,15,18}
                // + {28,32,36} = {42,51,60}
    if ((tmp_z[0] - 42) < eps && (tmp_z[1] - 51) < eps &&
        (tmp_z[2] - 60) < eps) {
      log.info(
          "✓ `add_lin_comb(mul_x, x, mul_y, y, mul_z, z)` method test passed");
      passed_counter++;
    } else {
  ...
  // Test in-place absolute value: x = |x|
  {
    ...
    if ((tmp_x[0] - 1) < eps && (tmp_x[1] - 2) < eps && (tmp_x[2] - 3) < eps) {
      ...
      log.error(
          "✗ `make_abs(x)` method test failed. Expected {1, 2, 3} but got {" +
          std::to_string(tmp_x[0]) + ", " + std::to_string(tmp_x[1]) + ", " +
          std::to_string(tmp_x[2]) + "}");
      failed_counter++;
    }
  ...
  // Test point-wise multiplication: x = x * (mul_y * y)
  {
    ...
    if ((tmp_x[0] - 8) < eps && (tmp_x[1] - 20) < eps &&
        (tmp_x[2] - 36) < eps) {
  ...
  // Test point-wise multiplication assignment: z = (mul_x * x) * (mul_y * y)
  {
    ...
    vec_space->mul_pointwise(2, tmp_x, 3, tmp_y,
                             tmp_z); // z = (2*{1,2,3}) * (3*{4,5,6}) = {2,4,6}
                                     // * {12,15,18} = {24,60,108}
    if ((tmp_z[0] - 24) < eps && (tmp_z[1] - 60) < eps &&
        (tmp_z[2] - 108) < eps) {
  ...
  // Test point-wise division assignment: z = (mul_x * x) / (mul_y * y)
  {
    ...
    if ((12 * tmp_z[0] - 2) < eps && (15 * tmp_z[1] - 4) < eps &&
        (18 * tmp_z[2] - 6) < eps) {
  ...
  // Test point-wise division: x = x / (mul_y * y)
  {
    ...
    if ((12 * tmp_x[0] - 1) < eps && (15 * tmp_x[1] - 2) < eps &&
        (18 * tmp_x[2] - 3) < eps) {
  ...
  //     std::vector<std::pair<size_t, size_t>> slices = {{0, 2}, {2, 3}}; //
  //     copy 0,1 then 2 -> all vec_space2->assign_slices(x2, slices, y2); if
  //     ((y2[0] - 1) < eps && (y2[1] - 2) < eps && (y2[2] - 3) < eps)
  //     {
  ...
  //     std::vector<std::pair<size_t, size_t>> skip = {{1, 2}}; // skip 1..2 ->
  //     keep 0 vec_space3->assign_skip_slices(x3, skip, y3); if ((y3[0] - 1) <
  //     eps)
  //     {
  //         log.info("✓ `assign_skip_slices(x, skip_slices, y)` method test
  //         passed"); passed_counter++;
  //     }
  //     else
  //     {
  //         log.error("✗ `assign_skip_slices(x, skip_slices, y)` method
