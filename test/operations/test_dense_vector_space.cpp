...
int main(int argc, char const* args[]) {
    ...
    using dense_vector_space_t = nmfd::operations::dense_vector_space<T, Dim, vector_type>;
    ...
    // Test vector space dimensions
    {
        ...
        if (dim == Dim) {
            ...
            log.error("✗ `size()` method test failed. Expected " + std::to_string(Dim) + " but returned " +
                      std::to_string(dim));
            failed_counter++;
        }
    }

    {
        ...
        if (vec_size == Dim) {
            ...
            log.error("✗ `get_size(x)` method test failed. Expected " + std::to_string(Dim) + " but returned " +
    ...
    // Test L2 norm
    {
        ...
        if (std::abs(norm - std::sqrt(14)) < eps) {
            ...
            log.error("✗ `norm(x)` method test failed. Expected " + std::to_string(std::sqrt(14)) + " but returned " +
                      std::to_string(norm));
            failed_counter++;
        }
    }
    ...
    // Test vector assignment: y = x
    {
        ...
        if ((tmp_y[0] - 1) < eps && (tmp_y[1] - 2) < eps && (tmp_y[2] - 3) < eps) {
            ...
            log.error("✗ `assign(x, y)` method test failed. Expected {1, 2, 3} but got {" + std::to_string(tmp_y[0]) +
    ...
    // Test linear combination assignment: z = mul_x * x + mul_y * y
    {
        ...
        vec_space->assign_lin_comb(2, tmp_x, 3, tmp_y,
    ...
    // Test in-place absolute value: x = |x|
    {
        ...
        if ((tmp_x[0] - 1) < eps && (tmp_x[1] - 2) < eps && (tmp_x[2] - 3) < eps) {
            ...
            log.error("✗ `make_abs(x)` method test failed. Expected {1, 2, 3} but got {" + std::to_string(tmp_x[0]) +
